 ...
 */
async function listAllUsers(request: Request, response: Response): Promise<void> {
	logger.info(request);
	...
	if (error == null) {
		...
		if (error instanceof DatabaseError) {
			...
			response.send({
				errorMessage:
					"Unexpected error while retrieving all users: " + error.message,
			});
		}
	}
}
 ...
 *                      a 500-level response if there is a system error
 */
async function getOneUser(request: Request, response: Response): Promise<void> {
	logger.info(request);
	...
	if (username == undefined) {
		...
		if (error != null) {
			...
			if (error instanceof InvalidInputError) {
				logger.trace(
					"Sending response status of 400 for GET /users/" +
						username +
						" request",
				);
				response.status(400);
				...
				logger.trace(
					"Sending response status of 500 for GET /users/" +
						username +
						" request",
				);
				response.status(500);
				...
				response.send({
					errorMessage:
						"Unexpected error while retrieving user: " + error.message,
				});
			}
		} else {
			...
			if (foundUser == null) {
				...
				const responseMessage =
					...
					foundUser.email;
				logger.trace(
					"Sending response status of 200 for GET /users/" +
						username +
						" request",
				);
...
async function createUser(request: Request, response: Response): Promise<void> {
	...
	if (error == null) {
		const responseMessage =
			"Successfully added user: " + username + " email: " + email;
...
async function deleteUser(request: Request, response: Response): Promise<void> {
	...
	if (error == null) {
		const responseMessage = "Successfully deleted user: " + username;
		logger.trace(
			"Sending response status of 200 for DELETE /users/" + username,
		);
		logger.info(responseMessage);
		response.status(200);
		...
		if (error instanceof InvalidInputError) {
			logger.trace(
				"Sending response status of 400 for DELETE /users/" + username,
			);
			response.status(400);
			response.send({ errorMessage: error.message });
		} else if (error instanceof DatabaseError) {
			logger.trace(
				"Sending response status of 500 for DELETE /users/" + username,
			);
			response.status(500);
			response.send({ errorMessage: error.message });
		} else {
...
async function updateUser(request: Request, response: Response): Promise<void> {
	...
	if (error == null) {
		const responseMessage =
			...
			email;
		logger.trace(
			"Sending response status of 200 for PUT /users/" + oldUsername,
		);
		logger.info(responseMessage);
		response.status(200);
		...
		if (error instanceof InvalidInputError) {
			logger.trace(
				"Sending response status of 400 for PUT /users/" + oldUsername,
			);
			response.status(400);
			response.send({ errorMessage: error.message });
		} else if (error instanceof DatabaseError) {
			logger.trace(
				"Sending response status of 500 for PUT /users/" + oldUsername,
			);
			response.status(500);
			response.send({ errorMessage: error.message });
		} else {
...
export async function checkCredentials(
	...
	if (error != null || !user) return false;
