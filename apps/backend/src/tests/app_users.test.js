...
test("POST /users success case", async () => {
	...
	expect(results[0].username.toLowerCase() == username.toLowerCase()).toBe(
		true,
	);
...
test("PUT /users success case", async () => {
	...
	expect(Array.isArray(results)).toBe(true);
	expect(results.length).toBe(1);
	expect(results[0].username.toLowerCase() == newUsername.toLowerCase()).toBe(
		true,
	);
	expect(results[0].email.toLowerCase() == newEmail.toLowerCase()).toBe(true);
	expect(results[0].password.toLowerCase() == newPassword.toLowerCase()).toBe(
		true,
	);
});
...
test("PUT /users failure case status 400: same username", async () => {
	...
	expect(Array.isArray(results)).toBe(true);
	expect(results.length).toBe(1);
	expect(results[0].username.toLowerCase() == oldUsername.toLowerCase()).toBe(
		true,
	);
	expect(results[0].email.toLowerCase() == email.toLowerCase()).toBe(true);
	expect(results[0].password.toLowerCase() == password.toLowerCase()).toBe(
		true,
	);
});
